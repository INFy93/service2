                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <span v-if="singleUser.id">Редактировать пользователя {{ singleUser.name }} </span>
                                    </DialogTitle>
                                </div>
                              <Form class="mt-6" @submit="changeUser" :validation-schema="schema" novalidate>
                                    <div class="mb-6">
                                        <label
                                            for="user_name"
                                    ...
                                    <div class="mb-6">
                                        <label
                                            for="user_pass"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                                            >Пароль (оставить пустым, если не нужно менять)</label
                                        >
                                        <Field
                                            type="password"
                                            v-model="singleUser.password"
                                            name="user_pass"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            required
                                        />
                                    ...
                                    <div class="mb-6" v-if="singleUser.password">
                                        <label
                                    ...
                                    <div class="mb-4">
                                        ...
                                        >
                                            <option
                                                ...
                                                :key="service.id"
                                                :value="service.id"
                                                :selected="service.id == singleUser.service_id"
                                            >
                                    ...
                                    <div class="mb-4 border-t">
                                        <div class="mt-3">
                                            <input type="checkbox" v-model="singleUser.is_admin" class="rounded focus:outline-none focus:ring dark:ring-offset-gray-600 dark:focus:ring-gray-500 border-gray-300 dark:border-gray-600 dark:bg-gray-500 dark:text-gray-400" :checked="singleUser.is_admin === 1">
...
export default {
   setup(props, { emit }) {
        ...
        const { singleUser, confirmPass, getSingleUser, updateUser } = useUsers();
        ...
        const schema = Yup.object({
            ...
            user_pass: Yup.lazy(value => {
                if (value !== "")
                {
                    return Yup.string().min(8, "Длина пароля должна быть не менее 8 символов")
                }

                return Yup.mixed().notRequired();
            }),
            user_pass_confirm: Yup.string().min(8, "Длина пароля должна быть не менее 8 символов").oneOf([Yup.ref("user_pass"), null], "Пароли не совпадают"),
        });
